...
///////
	...
	// 	selfSize = tamanoCirculo;
	
	// console.log(nombre + " " + selfSize)
...
///////////
	...
	var tamanoCirculo = 0.0;
		...
		// if(colorText >= 10 && colorText <= 100){
		// 	circleFruit.style.color = "black";
		// 	console.log("una cadena");
		// }

		//this.sizeCircle = tamanoCirculo;

		//selfSize = ((meses.length - nivelTemporada) * tamanoMes)/100;
	...
	function frame(){
		...
		if (biggerMe == tamanoCirculo*100) {
			...
			circleFruit.onclick = function() {
				var informacionScale = 0.0;
				var informacion = circleFruit;
				var animInfo = setInterval(frameInfo, 5);
				
				informacion.style.opacity =  "1";
				function frameInfo(){
							if(informacionScale == 50){
				...
				infoQueMeses.innerHTML = delMes + " - " + alMes;
					...
					console.log("informacion " + nombre + " " + informacionScale)
				
				if(informacionScale == 0){
					informacion.onclick = function() {
						var informacionScaleDown = informacionScale;
						var animInfo = setInterval(frameInfoClose, 5);
						function frameInfoClose(){
								if (informacionScaleDown == tamanoCirculo*10) {
									console.log("vengo de ser grande " + nombre + " " + tamanoCirculo*100);
									clearInterval(animInfo);
									informacionScale = 0.0;
								} else { 
										informacionScaleDown--;
										console.log("regreso a mi estado original " + nombre + " " + informacionScaleDown + " " + tamanoCirculo)
										informacion.style.transform = "scale(" + informacionScaleDown/10 + ", " + informacionScaleDown/10 + ")";
										informacion  = circleFruit;
				...
				var animRegreso = setInterval(frameInfo, 5);
				
				informacion.style.opacity =  "0.5";
				function frameInfo(){
							if(informacionScale == 50){
								clearInterval(animRegreso);
								console.log("UNO")
							}else{
								informacionScale++;
								informacion.style.transform = "scale(" + informacionScale/10 + ", " + informacionScale/10 + ")";
...
var frutaManzana = new fruta(1, "Manzana", [6, 7, 8, 9], "manzana.png", .1, "#61070E", 40, "a, c", "hierro fósforo potasio calcio", "#EFF8E7");
